...
#include "AdvancedLiftDragCustom.hh"
...
// Add a utility function to check the ALDC_VERBOSE environment variable
bool IsVerbose()
{
  const char* verbose = std::getenv("ALDC_VERBOSE");
  return verbose != nullptr && std::string(verbose) == "1";
}

// Add a function to compute the rotation matrix
gz::math::Matrix3d ComputeRotationMatrix(const gz::math::Vector3d &forward, const gz::math::Vector3d &upward)
{
  // Ensure the vectors are normalized
  gz::math::Vector3d xAxis = forward.Normalized();
  gz::math::Vector3d zAxis = upward.Normalized();
  gz::math::Vector3d yAxis = zAxis.Cross(xAxis).Normalized();
  zAxis = xAxis.Cross(yAxis).Normalized();

  // Construct the rotation matrix
  return gz::math::Matrix3d(xAxis[0], xAxis[1], xAxis[2],
                           ...
                           const sdf::ElementPtr &_sdf)
{
  ...
  */
  ...
  if (_sdf->HasElement("link_name"))
  {
    ...
    if (entities.empty())
    {
      gzerr << "Link with name[" << linkName << "] not found. "
             << "The AdvancedLiftDragCustom will not generate forces.\n";
      this->validConfig = false;
      return;
    }
    else if (entities.size() > 1)
  ...
  {
    gzerr << "AdvancedLiftDragCustom system requires the 'link_name' parameter.\n";
    this->validConfig = false;
    return;
  }
  ...
  if (IsVerbose())
  {
    std::cout << "Custom Lift Drag Loaded" << std::endl;
  }
}

//////////////////////////////////////////////////
AdvancedLiftDragCustom::AdvancedLiftDragCustom()
...
void AdvancedLiftDragPrivate::Update(EntityComponentManager &_ecm)
{
  ...
  if (windLinearVel != nullptr){
    ...
    if (IsVerbose())
    {
      std::cout << "windLinearVel: " << windLinearVel->Data() << std::endl;
    }
  }
  else {
    if (IsVerbose())
    {
      std::cout << "windLinearVel is nullptr" << std::endl;
    }
  }
  ...
  if (IsVerbose())
  {
    std::cout << "AngVel: " << AngVel->Data() << std::endl;
    std::cout << "body_x_axis: " << body_x_axis << std::endl;
    std::cout << "body_y_axis: " << body_y_axis << std::endl;
    std::cout << "body_z_axis: " << body_z_axis << std::endl;
  }



  // Use the rotation matrix to transform the angular velocity
  gz::math::Matrix3d rotationMatrix = ComputeRotationMatrix(this->forward, this->upward);
  gz::math::Vector3d transformedAngVel = rotationMatrix * AngVel->Data();

  // Extract the roll, pitch, and yaw rates from the transformed angular velocity
  double rr = transformedAngVel.X();  // Roll rate
  double pr = transformedAngVel.Y();  // Pitch rate
  double yr = transformedAngVel.Z();  // Yaw rate
  ...
  if (IsVerbose())
  {
    std::cout << "dyn_pres: " << dyn_pres << "| vLD:" << speedInLDPlane << std::endl;
  }
  ...
  if (IsVerbose())
  {
    std::cout << "pr: " << pr << "| rr: " << rr <<"| yr: " << yr << std::endl;
    std::cout << "pm: " << pm << "| rm: " << rm << "| ym: " << ym << std::endl;
  }
...
//////////////////////////////////////////////////
void AdvancedLiftDragCustom::Configure(const Entity &_entity,
                         ...
                         EntityComponentManager &_ecm, EventManager &)
{
  ...
  gzwarn << "AVL Custom Configure" << std::endl;
}

//////////////////////////////////////////////////
void AdvancedLiftDragCustom::PreUpdate(const UpdateInfo &_info,
  EntityComponentManager &_ecm)
{
  GZ_PROFILE("AdvancedLiftDragCustom::PreUpdate");
...
GZ_ADD_PLUGIN(AdvancedLiftDragCustom,
                    System,
                    AdvancedLiftDragCustom::ISystemConfigure,
                    AdvancedLiftDragCustom::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(AdvancedLiftDragCustom,
                    "gz::sim::systems::AdvancedLiftDragCustom")
