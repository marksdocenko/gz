...
#include "MulticopterMotorModelOrigin.hh"
...
class gz::sim::systems::MulticopterMotorModelOriginPrivate
{
  ...
  /// \brief Maximum relative airspeed at which the motor model produces no thrust.
  public: double maxRelativeAirspeed = 185;

  /// \brief Relative airspeed at which the motor linearly starts to lose efficiency.
  public: double relativeAirspeed0 = 109;
...
//////////////////////////////////////////////////
MulticopterMotorModelOrigin::MulticopterMotorModelOrigin()
  : dataPtr(std::make_unique<MulticopterMotorModelOriginPrivate>())
{
}

//////////////////////////////////////////////////
void MulticopterMotorModelOrigin::Configure(const Entity &_entity,
    ...
    EventManager &/*_eventMgr*/)
{
  ...
  {
    gzerr << "MulticopterMotorModelOrigin plugin should be attached to a model "
  ...
  {
    gzerr << "MulticopterMotorModelOrigin found an empty jointName parameter. "
  ...
  {
    gzerr << "MulticopterMotorModelOrigin found an empty linkName parameter. "
           << "Failed to initialize.";
    return;
  }
  ...
  sdfClone->Get<double>("maxRelativeAirspeed",
      this->dataPtr->maxRelativeAirspeed, this->dataPtr->maxRelativeAirspeed);
  sdfClone->Get<double>("relativeAirspeed0",
      this->dataPtr->relativeAirspeed0, this->dataPtr->relativeAirspeed0);
  ...
  this->dataPtr->node.Subscribe(topic,
      &MulticopterMotorModelOriginPrivate::OnActuatorMsg, this->dataPtr.get());
}

//////////////////////////////////////////////////
void MulticopterMotorModelOrigin::PreUpdate(const UpdateInfo &_info,
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("MulticopterMotorModelOrigin::PreUpdate");
...
//////////////////////////////////////////////////
void MulticopterMotorModelOriginPrivate::OnActuatorMsg(
...
//////////////////////////////////////////////////
void MulticopterMotorModelOriginPrivate::UpdateForcesAndMoments(
    EntityComponentManager &_ecm)
{
  GZ_PROFILE("MulticopterMotorModelOriginPrivate::UpdateForcesAndMoments");
  ...
  switch (this->motorType)
  {
    ...
    default:  // MotorType::kVelocity
    {
      ...
      Vector3 velocityParallelToRotorAxis = (relativeWindVelocityWorld.Dot(jointAxis)) * jointAxis;
      Vector3 bodyVelocityPerpendicular =
          relativeWindVelocityWorld -
          velocityParallelToRotorAxis;
      Vector3 airDrag = -std::abs(realMotorVelocity) *
                               this->rotorDragCoefficient *
                               bodyVelocityPerpendicular;

      double scalar = 1;
      if (velocityParallelToRotorAxis.Length() > this->relativeAirspeed0) {
        scalar = 1 - (velocityParallelToRotorAxis.Length() - this->relativeAirspeed0) / (this->maxRelativeAirspeed - this->relativeAirspeed0);
      }
      scalar = gz::math::clamp(scalar, 0.0, 1.0);

      // Apply a force to the link.
      link.AddWorldForce(_ecm,
                         worldPose->Rot().RotateVector(Vector3(0, 0, thrust * scalar)));
...
GZ_ADD_PLUGIN(systems::MulticopterMotorModelOrigin,
                    gz::sim::System,
                    MulticopterMotorModelOrigin::ISystemConfigure,
                    MulticopterMotorModelOrigin::ISystemPreUpdate)

GZ_ADD_PLUGIN_ALIAS(systems::MulticopterMotorModelOrigin,
                          "gz::sim::systems::MulticopterMotorModelOrigin")
